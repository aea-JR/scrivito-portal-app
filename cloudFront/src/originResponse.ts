...
export async function handler(
  ...
  if(request.querystring.includes("status-code=404-not-found") || request.uri == "/404" || request.uri == "/404.html" ) {
...
async function movedTo404(domain: string, readCloudFrontOnlyHeaders: AWSLambda.CloudFrontHeaders) {
  console.log("GET /404.html (Status-Code: 404), but deliver index.html until there is a custom 404.html");
  let host = domain;
  if (basicAuth) {
  ...
  const {
    headers,
    body
  } = await httpsGet(`https://${host}/index.html`);
...
const ADDITIONAL_CLOUD_FRONT_HEADERS = preval.require<
  AWSLambda.CloudFrontHeaders
>("./generateAdditionalHeaders.js");
